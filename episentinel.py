...
def main():
    ...
    CSV_PATH = "covid_tweets.csv"  # replace with your Kaggle file
    if os.path.exists(CSV_PATH):
        df = pd.read_csv(CSV_PATH)
        # adapt common Kaggle column names
        if "created_at" in df.columns: df = df.rename(columns={"created_at":"date"})
        if "tweet" in df.columns: df = df.rename(columns={"tweet":"text"})
        df["date"] = pd.to_datetime(df["date"])
        df["day"] = (df["date"] - df["date"].min()).dt.days
        df["phase"] = "real_data"
        print(f"  Loaded Kaggle dataset: {len(df):,} posts")
    else:
...
def run_streamlit_app():
    ...
    if st.button("Run EpiSentinel"):
        ...
        elif os.path.exists("covid_tweets.csv"):
            CSV_PATH = "covid_tweets.csv"
            st.success("Using local covid_tweets.csv")
        else:
            CSV_PATH = None
        ...
        if CSV_PATH is not None:
            ...
            df["phase"]="calm"
            df.loc[df["compound"]<-0.2,"phase"]="outbreak"
            df.loc[df["compound"]>0.2,"phase"]="recovery"

            daily_agg=(df.groupby("day").agg(
                date=("date","first"),
                phase=("phase",lambda x:x.mode().iloc[0] if len(x.mode()) else "calm"),
                mean_adjusted=("compound","mean"),
                mean_positive=("pos","mean"),
                mean_negative=("neg","mean"),
                mean_neutral=("neu","mean"),
                post_count=("text","count"),
                risk_pct_critical=("compound",lambda x:(x<-0.5).mean()),
                risk_pct_high=("compound",lambda x:((x>=-0.5)&(x<-0.2)).mean())
            ).reset_index())
